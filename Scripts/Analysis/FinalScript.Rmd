...
res <- simulateResiduals(CategoryGLM251)

jpeg("Scripts/Analysis/Figures/residuals.jpg",
...
data_pred_asthma <- data.frame(asthma_sim, population_vector, riverine_vector)
...
prediction_asthma <- predict(CategoryGLM251, newdata = data_pred_asthma, type = "response",
                        se.fit = TRUE)
# Pull out predictions.
data_fig_asthma <- data.frame(data_pred_asthma, prediction_asthma$fit,prediction_asthma$se.fit)

colnames(data_fig_asthma) <- c("asthma","population", "riverine", "probability", "se")
...
data_pred_pop <- data.frame(asthma_vector, pop_sim, riverine_vector)

colnames(data_pred_pop) <- c("current_adult_asthma","population_log", "riverine_flooding_annualized_frequency_log")

# Use the original model to predict outcomes.
prediction_population <- predict(CategoryGLM251, newdata = data_pred_pop, type = "response",
                        se.fit = TRUE)
# Pull out predictions.
data_fig_pop <- data.frame(data_pred_pop, prediction_population$fit,prediction_population$se.fit)

colnames(data_fig_pop) <- c("asthma","population", "riverine","probability", "se")
...
data_pred_riverine <- data.frame(asthma_vector, population_vector, riverine_sim)

colnames(data_pred_riverine) <- c("current_adult_asthma","population_log", "riverine_flooding_annualized_frequency_log")

# Use the original model to predict outcomes.
prediction_riverine <- predict(CategoryGLM251, newdata = data_pred_riverine, type = "response",
                        se.fit = TRUE)
# Pull out predictions.
data_fig_riverine <- data.frame(data_pred_riverine, prediction_riverine$fit,prediction_riverine$se.fit)

colnames(data_fig_riverine) <- c("asthma","population", "riverine","probability", "se")
