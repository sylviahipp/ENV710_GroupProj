...
## CVI
```{r CVI Data Wrangling, echo=TRUE, message=FALSE, warning=FALSE}
...
# Get population at the census tract and county level --------------------------------

# census-tract population for population-weighted averages
...
# county population to use for analysis
...
# log adjust across all cvi metrics
...
title <- ggdraw() +
  draw_label("Distribution of CVI Metrics and Population", 
...
```{r eGRID Data Wrangling, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r Plant Categories Map, echo=TRUE, message=FALSE, warning=FALSE}
southern_fps <- c(
...
corr_plot <- cvi_egrid_df %>% 
  mutate(riverine_flooding_annualized_frequency_log = case_when(
    riverine_flooding_annualized_frequency_log==-Inf ~ -1000000,
    TRUE ~ riverine_flooding_annualized_frequency_log)) %>% 
...
unique(cvi_egrid_join$Category)

windsolar <-
  cvi_egrid_join %>% 
  filter(Category == "Contains solar/wind")

solarwindff  <-
  cvi_egrid_join %>% 
  filter(Category == "Contains FF and solar/wind")

ff <-
  cvi_egrid_join %>% 
  filter(Category == "Contains FF")

noplant<-
  cvi_egrid_join %>% 
  filter(Category == "No plant")

noplantflood <- riverine_notinfinity %>% 
  filter(Category == "No plant")

windsolarflood <- riverine_notinfinity %>% 
  filter(Category == "Contains solar/wind")

windsolarflood_summary <- data.frame(
  mean = sapply(windsolarflood[sapply(windsolarflood, is.numeric)], mean, na.rm = TRUE),
  sd   = sapply(windsolarflood[sapply(windsolarflood, is.numeric)], sd, na.rm = TRUE)
)

noplantflood_summary <- data.frame(
  mean = sapply(noplantflood[sapply(noplantflood, is.numeric)], mean, na.rm = TRUE),
  sd   = sapply(noplantflood[sapply(noplantflood, is.numeric)], sd, na.rm = TRUE)
)

windsolar_summary <- data.frame(
  mean = sapply(windsolar[sapply(windsolar, is.numeric)], mean, na.rm = TRUE),
  sd   = sapply(windsolar[sapply(windsolar, is.numeric)], sd, na.rm = TRUE)
)

windsolarff_summary <- data.frame(
  mean = sapply(solarwindff[sapply(solarwindff, is.numeric)], mean, na.rm = TRUE),
  sd   = sapply(solarwindff[sapply(solarwindff, is.numeric)], sd, na.rm = TRUE)
)


ff_summary <- data.frame(
  mean = sapply(ff[sapply(ff, is.numeric)], mean, na.rm = TRUE),
  sd   = sapply(ff[sapply(ff, is.numeric)], sd, na.rm = TRUE)
)

noplant_summary <- data.frame(
  mean = sapply(noplant[sapply(noplant, is.numeric)], mean, na.rm = TRUE),
  sd   = sapply(noplant[sapply(noplant, is.numeric)], sd, na.rm = TRUE)
)
...
level1AIC <- as.data.frame(rbind(AIC(CategoryGLM1),
                                 AIC(CategoryGLM2),
                                 AIC(CategoryGLM3),
                                 AIC(CategoryGLM4),
                                 AIC(CategoryGLM5)))
...
CategoryGLM253 <- glm(category_GLM ~ current_adult_asthma + riverine_flooding_annualized_frequency_log + below_poverty,
                      data = data1,
                      family = "binomial")
CategoryGLM254 <- glm(category_GLM ~ current_adult_asthma + riverine_flooding_annualized_frequency_log 
                      + no_high_school_diploma,
                      data = data1,
                      family = "binomial")
...
level4AIC <- as.data.frame(rbind(AIC(CategoryGLM2513),
                                 AIC(CategoryGLM2514)))
min(level4AIC) # it is only fractional improvement to add no_high_school_diploma
# We decide this is not sufficient improvement to warrant additional complexity
...
# final model: category_GLM ~ current_adult_asthma + riverine_flooding_annualized_frequency_log + population_log

```

### Model visualization

```{r visualize model, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r asthma vis, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r population vis, echo=TRUE, message=FALSE, warning=FALSE}
# Simulate population
...
```{r flooding vis, echo=TRUE, message=FALSE, warning=FALSE}
# Simulate riverine
